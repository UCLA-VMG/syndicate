...
CameraPtr getCam(SystemPtr system, CameraList camList, string serialNumber) {
    ...
    // Configure rgb connected camera to use User Set 1 and register image events
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    CameraPtr rgb_cam = getCam(system, camList, serialNumber);
    // Initialize camera
    rgb_cam->Init();
    // Get NodeMap and nodeMapTLDevice
    INodeMap& nodeMap = rgb_cam->GetNodeMap();
    INodeMap& nodeMapTLDevice = rgb_cam->GetTLDeviceNodeMap();

    try {
        // Configure camera
        if (!configure(rgb_cam, nodeMap, fps, height, width)) {
            cout << "Camera configuration for device " << serialNumber << " unsuccessful, aborting...";
            return -1;
        }
        // Acquire images and save into vector
        result = result | AcquireImages(rgb_cam, nodeMap, nodeMapTLDevice, num_frames);
        // Deinitialize camera
        rgb_cam->DeInit();
    }
    catch (Spinnaker::Exception& e) {
