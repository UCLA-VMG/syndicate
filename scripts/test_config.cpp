...
void debug_settings::load(const std::string &filename)
{
    ...
    BOOST_FOREACH( const ptree::value_type & ct, pt.get_child("sensor_stack") )
    {
        std::cout << "I am in the child of child tree " << ct.first << ", " << ct.second.get<std::string>("<xmlattr>.id")  << std::endl;
        ...
        // m_file = ct.first.get<std::string>("exe_path");
        ...
        // m_level = ct.get<int>("sensor.sensor_stack.level");
        ...
        //BOOST_FOREACH(ptree::value_type &v, pt.get_child("sensor_stack.modules"))
...
void debug_settings::save(const std::string &filename)
{
    ...
    pt.put("sensor_stack.filename", m_file);

    // Put debug level in property tree
    pt.put("sensor_stack.level", m_level);
...
int main()
{
    ...
    // {
    //     debug_settings ds;
    //     ds.load("C:/Users/111/Desktop/repos/syndicate/scripts/test_mx800.xml");
    //     ds.save("C:/Users/111/Desktop/repos/syndicate/scripts/debug_settings_out.xml");
    //     std::cout << "Success\n";
    // }
    ...
    using boost::property_tree::ptree;
    ptree tree;
    std::string XML_PATH1("C:/Users/111/Desktop/repos/syndicate/scripts/test_mx800.xml");
    std::string ATTR_SET("sensor");
    read_xml(XML_PATH1, tree);
    // const ptree & formats = tree.get_child("sensor_stack", empty_ptree());
    BOOST_FOREACH(ptree::value_type & f, tree.get_child("sensor_stack")){
        ...
        if(at == ATTR_SET)
        {
            ...
            // std::cout << "  EXE Path: " << f.second.get<std::string>("exe_path") << std::endl;
            // std::cout << "  Level: " << f.second.get<int>("level") << std::endl;
            if(f.second.find("lol") == f.second.not_found())
                std::cout << "lol not here\n";
        }
        else
        {
